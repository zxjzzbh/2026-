...
- 视觉：WXSJ-H65HD USB 广角摄像头；外接设备已完成 Windows 出图与采集测试，详见视觉模块的相机测试记录，树莓派端仍待测试。
- 姿态：WIT-Motion IMU，UART 通信，具体型号、电平与协议待确认。
- 通信：现有蜂窝/5G 模块，具体型号、接口和联网方式待确认。
- 动力与供电：已有电机、ESC、约 12 V 锂电池和 DC-DC；实际电压、电流能力与接法以实测记录为准。
- 下位机：已确认使用 STM32 F4 系列，负责舵机/ESC、PWM、编码器和闭环；具体型号、开发板、电平和串口协议待确定。
...
| 模块 | 当前状态 | 下一项可交付成果 |
|---|---|---|
| 视觉 | v0.1 离线框架已实现，见模块 README | 实拍数据、比赛模型微调与树莓派验证 |
